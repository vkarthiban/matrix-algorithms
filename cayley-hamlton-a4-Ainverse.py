class caly():
    ...
    ###############################################################
    def Ainverse(self,matx0,matx1,matx2,Ainvalue):
        matxb = [[0,1,2],[0,1,2],[0,1,2]]
        matxc = [[0,1,2],[0,1,2],[0,1,2]]
        for cnt1 in range(len(matx1)):
            for cnt2 in range(len(matx1)):
                matxb[cnt1][cnt2] =(matx0[cnt1][cnt2]+matx1[cnt1][cnt2]+matx2[cnt1][cnt2])
                matxc[cnt1][cnt2] =int((matx0[cnt1][cnt2]+matx1[cnt1][cnt2]+matx2[cnt1][cnt2])/Ainvalue)
        ainverse = [matxb,matxc]
        return ainverse
###############################################################################        

    def A4Mul(self,matx0,matx1,matx2):
        matA4mul = [[0,1,2],[0,1,2],[0,1,2]]
        for cnt1 in range(len(matx1)):
            for cnt2 in range(len(matx1)):
                matA4mul[cnt1][cnt2] =(matx0[cnt1][cnt2]+matx1[cnt1][cnt2]+matx2[cnt1][cnt2])
        return matA4mul
    ...
    def mainController(self,mat):
        ...
        eigvalA = self.valMatMul(eignls1[0],mat)
        eigvaliden =self.valMatMul(eignls1[1],indmat)
        maxing =self.Ainverse(A2,eigvalA,eigvaliden,eignls1[2])
        print("Ainverse matrix without diveded")
        self.matrixprint(maxing[0])
        print("Ainverse matrix dived final value")
        self.matrixprint(maxing[1])

    ##################################################A4 finding################################

        eignls2 = [(eignls1[0]*-1),(eignls1[1]*-1),(eignls1[2]*-1)]

        A4S1=(eignls2[0]*eignls2[0])+eignls2[1]
        A4S2=(eignls2[0]*eignls2[1])+eignls2[2]
        A4S3 = eignls2[0]*eignls2[2]
        A4mat1 = self.valMatMul(A4S1,A2)
        A4mat2 = self.valMatMul(A4S2,mat)
        A4mat3 = self.valMatMul(A4S3,indmat)
        A4matrix = self.A4Mul(A4mat1,A4mat2,A4mat3)
        print("A4 matrix",)
        self.matrixprint(A4matrix)
